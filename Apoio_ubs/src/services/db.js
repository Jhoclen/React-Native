...
const db = openDatabaseSync('agenda.db');
...
export async function initDatabaseIfNeeded() {
  ...
  const seeded = await AsyncStorage.getItem('@agenda_seeded_v1');
  if (!seeded) {
    await seedSlotsFromGenerator();
    await AsyncStorage.setItem('@agenda_seeded_v1', '1');
  }
}
...
async function seedSlotsFromGenerator() {
  ...
  await db.withTransactionAsync(async () => {
    for (const slot of generated) {
      await db.runAsync(
        `INSERT INTO slots (data, hora, servico, capacidadeMax, reservas)
         VALUES (?, ?, ?, ?, ?);`,
        [slot.data, slot.hora, slot.servico, slot.capacidadeMax, 0]
      );
    }
  });
}
...
export async function getUniqueDates(servico) {
  const rows = await db.getAllAsync(
    `SELECT DISTINCT data FROM slots WHERE servico = ? ORDER BY data;`,
    [servico]
  );
...
export async function reservarSlot(slotId, { idBooking, nome, cpf, prioridade }) {
  return db.withTransactionAsync(async () => {
    ...
    await db.runAsync(
      `INSERT INTO bookings (id, slot_id, nome, cpf, prioridade, created_at)
       VALUES (?, ?, ?, ?, ?, ?);`,
      [idBooking, slotId, nome, cpf, prioridade || null, new Date().toISOString()]
    );
